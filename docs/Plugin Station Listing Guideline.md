...
We will open the opportunity to list new plugins on Plugin Station _once a month_. Please get in touch with your Allxon contact window for upcoming listing dates. To ensure the quality and safety of our Plugin Station, developers must submit the necessary information at least **three weeks** before the listing date.

![ListingTimeline](_img/Listing_Timeline.svg)

There are two things that we will do when we receive your plugin submission.

- [Plugin Review](#submit-your-plugin): To ensure that the submitted plugins comply with our policies and guidelines, and meet legal requirements and user experience standards, we will review each plugin, which usually takes _5-10 working days_. If any issues are found during the reviewing process, developers must address them immediately and resubmit the plugin for review.

- [Listing Content Curation](_img/Listing_Content_A.svg): Once your plugin has been successfully reviewed, we will prepare the plugin’s product page based on your submitting content, which usually takes _3-5 working days_.
